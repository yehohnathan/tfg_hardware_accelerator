...
# 2 "<built-in>" 2
# 1 "../src/forward_fw.cpp" 2

# 1 "../src/forward_fw.hpp" 1
...
# 3 "../src/forward_fw.hpp" 2
# 1 "C:/Xilinx/Vitis/2024.2/common/technology/autopilot\\ap_int.h" 1
# 4 "../src/forward_fw.hpp" 2
...
# 39 "C:/Xilinx/Vitis/2024.2/tps/mingw/8.3.0/win64.o/nt\\lib\\gcc\\x86_64-w64-mingw32\\8.3.0\\include\\c++\\cstdint" 2 3
...
__attribute__((sdx_kernel("train_step", 0))) void train_step(const bin_t img_pos[N_INPUT],
                const bin_t img_neg[N_INPUT],
                int last_sample,
                int sample_idx,
                volatile ap_uint<4>& leds_port,
                bin_t W1_out[N_HIDDEN][N_INPUT],
                bin_t W2_out[N_OUTPUT][N_HIDDEN]);
# 3 "../src/forward_fw.cpp" 2

# 1 "C:/Xilinx/Vitis/2024.2/common/technology/autopilot\\ap_int.h" 1
# 5 "../src/forward_fw.cpp" 2





typedef ap_fixed<8,8> bin_t;
typedef ap_fixed<12,12> acc_t;
...
acc_t computeGoodness(const bin_t vec[], int size) {
    ...
    VITIS_LOOP_35_1: for(int i = 0; i < size; i++) {
        acc_t v = (acc_t)vec[i];
        sq += v * v;
    }
    return sq;
...
#pragma HLS PIPELINE II=1
 ...
 VITIS_LOOP_52_1: for(int j = 0; j < N_HIDDEN; j++) {
        acc_t sum = 0;

        VITIS_LOOP_55_2: for(int i = 0; i < N_INPUT; i++) {
...
#pragma HLS PIPELINE II=1
 ...
 VITIS_LOOP_70_1: for(int k = 0; k < N_OUTPUT; k++) {
        acc_t sum = 0;
        VITIS_LOOP_72_2: for(int j = 0; j < N_HIDDEN; j++) {
...
#pragma HLS PIPELINE II=1
 ...
 VITIS_LOOP_92_1: for(int j = 0; j < N_HIDDEN; j++) {
        bin_t delta = out_pos[j] - out_neg[j];
        VITIS_LOOP_94_2: for(int i = 0; i < N_INPUT; i++) {
...
#pragma HLS INTERFACE ap_vld port=leds_port
 ...
 static bin_t hidden_pos[N_HIDDEN], hidden_neg[N_HIDDEN];
    ...
    VITIS_LOOP_136_1: for(int j=0; j < N_HIDDEN; j++)
        VITIS_LOOP_137_2: for(int i=0; i<N_INPUT; i++)
            W1_out[j][i] = W1[j][i];

    VITIS_LOOP_140_3: for(int k=0; k < N_OUTPUT; k++)
        VITIS_LOOP_141_4: for(int j=0; j < N_HIDDEN; j++)
            W2_out[k][j] = W2[k][j];


    unsigned lvl = (sample_idx * 15u) / (NUM_IMAGES - 1u);
    leds_port = last_sample ? ap_uint<4>(~lvl) : ap_uint<4>(lvl);
}
